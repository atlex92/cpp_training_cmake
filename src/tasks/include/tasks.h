...
*/
    ...
    int findMin(const vector<int>& nums) {
        ...
        if(ret > nums.back()) {
            auto it{FinfFirstDesc(nums.begin(), nums.end())};
            ret = *it;
            // cout << "ret = " << ret << endl;
        }
...
private:
    vector<int>::const_iterator FinfFirstDesc(vector<int>::const_iterator start, vector<int>::const_iterator end) {

        // cout << "searching desc from " << *start << " to " << *end << endl;
        ...
        if(len == 1) {
            ...
            auto middle{start + len / 2};
            if(*start > *middle) {
                ret = FinfFirstDesc(start, middle);
            }
            else {
                ret = FinfFirstDesc(middle, end);
            }
        }
